...
async saveResume(
  ...
  if (!image) {
    throw new HttpException('An image file is required!', HttpStatus.BAD_REQUEST);
  }
  if (!pdf) {
  ...
  saveResume.experience = extractedDataFromPDF.experience || saveResume.experience;
  ...
  saveResume.dateOfBirth = extractedDataFromPDF.dateOfBirth || saveResume.dateOfBirth;
